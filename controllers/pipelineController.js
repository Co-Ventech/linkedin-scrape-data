...
// Get company pipeline (supports both company users and super admin)
const getCompanyPipeline = async (req, res) => {
  try {
    let companyId;
    
    // Super admin can specify companyId in params, others use their own company
    if (req.user.role === 'super_admin' && req.params.companyId) {
    ...
    if (!pipeline) {
      // Create default pipeline if none exists
      pipeline = await CompanyPipeline.createDefaultPipeline(companyId, req.user._id);
    }
...
// Update company pipeline (supports both company admin and super admin)
const updateCompanyPipeline = async (req, res) => {
  try {
    let companyId;
    ...
    // Simplified validation - only check for duplicate names
    if (statusStages && statusStages.length > 0) {
      const stageNames = statusStages.map(stage => stage.name);
      const uniqueNames = new Set(stageNames);
    ...
    const pipeline = await CompanyPipeline.findOneAndUpdate(
      { companyId },
      {
        ...
        useCustomPipeline: useCustomPipeline || false,
        lastModifiedBy: req.user._id
      },
      { new: true, upsert: true }
    );
...
const getAvailableTransitions = async (req, res) => {
  try {
    ...
    // Get all other stages as available transitions (simplified)
    const availableTransitions = pipeline.statusStages.filter(stage => 
      stage.name !== job.currentStatus
    );
...
const getPipelineAnalytics = async (req, res) => {
  try {
    ...
    // Super admin can specify companyId in params
    if (req.user.role === 'super_admin' && req.params.companyId) {
      companyId = req.params.companyId;
    }
...
// Helper function for default transitions
const getDefaultTransitions = (currentStatus) => {
  const defaultStages = [
    'not_engaged', 'applied', 'engaged', 'interview', 'offer', 'onboard', 'rejected', 'archived'
  ];
  return defaultStages.filter(stage => stage !== currentStatus);
};


const getCompanyStatusStages = async (req, res) => {
  try {
    let companyId;
    ...
    if (!pipeline) {
      return res.json({ statusStages: [] });
    }
    
    res.json({
      statusStages: pipeline.statusStages.map(stage => ({
        name: stage.name,
        displayName: stage.displayName
      }))
    });
  } catch (error) {
    res.status(500).json({ error: 'Failed to get status stages' });
  }
};
